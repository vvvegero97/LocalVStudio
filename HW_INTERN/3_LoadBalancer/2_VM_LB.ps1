...
$mylb = "MyLoadBalancer"
az network lb create -g $myrg --name $mylb --frontend-ip-name MyFrontEndPool --backend-pool-name MyBackEndPool --public-ip-address $mypub

#create lb health probe
Write-Host "Creating Health Probe..."
az network lb probe create -g $myrg --lb-name $mylb --name MyHealthProbe --protocol tcp --port 80

#create load balancer rule
Write-Host "Creating LB rule..."
az network lb rule create -g $myrg --lb-name $mylb --name MyLoadBalancerRule --protocol tcp --frontend-port 80 --backend-port 80 --frontend-ip-name MyFrontEndPool --backend-pool-name MyBackEndPool --probe-name MyHealthProbe
...
az network nsg rule create -g $myrg --nsg-name MyNSG --name MyNSGRule --priority 1001 --protocol tcp --destination-port-range 80

#create interfaces
for ( $i = 1 ; $i -le 3 ; $i++) {
    Write-Host "Creating Nic$i..."
    az network nic create -g $myrg --name MyNic$i --vnet-name MyVnet --subnet MySubnet --network-security-group MyNSG --lb-name $mylb --lb-address-pools MyBackEndPool
}
...
for ( $i = 1 ; $i -le 3 ; $i++) {
    ...
    az vm create -g $myrg --name MyVM$i --availability-set MyAvailSet --nics MyNic$i --image UbuntuLTS --admin-username azureuser --generate-ssh-keys --custom-data cloud-init.txt --no-wait
}
#test load balancer
Write-Host "Cretion Completed! Input this Public IP-address in browser to test:"
az network public-ip show --resource-group MyLoadBalancerRG --name myPublicIP --query [ipAddress] --output tsv
